...
const detailOpen = ref(false); const detailLoading = ref(false); const detail = ref<{ article: Article; reposts: Publication[]; runs: SearchRun[]; candidates: Candidate[] } | null>(null); const ownedChannels = ref<OwnedChannel[]>([]);
const reviewDialogOpen = ref(false); const reviewingCandidate = ref<Candidate | null>(null); const reviewAction = ref<ReviewAction>("CONFIRM_REPOST"); const reviewReason = ref(""); const reviewOwnedChannelId = ref<number | null>(null); const reviewSubmitting = ref(false);
const reviewMode = ref<ReviewMode>("NEW");
...
function relationLabel(value: string): string { return ({ REPOST: "外部转载", OWNED: "自有渠道", REVIEW_REQUIRED: "待复核" } as Record<string, string>)[value] || "候选待复核"; }
...
async function openDetail(article: Article): Promise<void> { detailOpen.value = true; detailLoading.value = true; detail.value = null; try { const [data, reading] = await Promise.all([api<{ reposts: Publication[]; search_runs: SearchRun[]; candidates: Candidate[] }>(`/articles/${article.id}/discovered-publications?as_of=${encodeURIComponent(asOf.value)}`), api<{ channels: OwnedChannel[] }>("/reading-monitor/owned-publications")]); detail.value = { article, reposts: data.reposts, runs: data.search_runs, candidates: data.candidates }; ownedChannels.value = reading.channels; } catch (caught) { error.value = caught instanceof Error ? caught.message : "详情加载失败"; } finally { detailLoading.value = false; } }
function reviewDialogTitle(): string { if (reviewMode.value === "MODIFY") return "修改人工复核"; return reviewAction.value === "CONFIRM_REPOST" ? "确认外部转载" : reviewAction.value === "CONFIRM_OWNED" ? "归入阅读量" : "排除候选"; }
function reviewCandidate(candidate: Candidate, action: ReviewAction): void { reviewingCandidate.value = candidate; reviewMode.value = "NEW"; reviewAction.value = action; reviewReason.value = ""; reviewOwnedChannelId.value = null; reviewDialogOpen.value = true; }
function modifyCandidateReview(candidate: Candidate): void { if (!candidate.manual_review_action) return; reviewingCandidate.value = candidate; reviewMode.value = "MODIFY"; reviewAction.value = candidate.manual_review_action; reviewReason.value = ""; reviewOwnedChannelId.value = candidate.owned_channel_id; reviewDialogOpen.value = true; }
async function submitCandidateReview(): Promise<void> { const candidate = reviewingCandidate.value; const reason = reviewReason.value.trim(); if (!candidate) return; if (!reason) { ElMessage.warning(reviewMode.value === "MODIFY" ? "必须填写修改原因" : "必须填写复核原因"); return; } if (reviewAction.value === "CONFIRM_OWNED" && !reviewOwnedChannelId.value) { ElMessage.warning("请选择自有渠道"); return; } reviewSubmitting.value = true; error.value = ""; try { await api(`/search-candidates/${candidate.id}/review`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: reviewAction.value, reason, ...(reviewAction.value === "CONFIRM_OWNED" ? { owned_channel_id: reviewOwnedChannelId.value } : {}) }) }); ElMessage.success(reviewMode.value === "MODIFY" ? "人工复核已修改" : reviewAction.value === "CONFIRM_REPOST" ? "已确认外部转载" : reviewAction.value === "CONFIRM_OWNED" ? "已归入阅读量检测" : "候选已排除"); reviewDialogOpen.value = false; if (detail.value) await openDetail(detail.value.article); await load(false); } catch (caught) { error.value = caught instanceof Error ? caught.message : "复核提交失败"; } finally { reviewSubmitting.value = false; } }
...
<template>
  <section class="monitoring-panel">
    ...
    <el-drawer v-model="detailOpen" size="76%" :title="detail?.article.title || '监测详情'">
      <el-skeleton v-if="detailLoading" :rows="6" animated />
      <template v-else-if="detail">
        <h3>外部转载结果</h3>
        <el-empty v-if="!detail.reposts.length" description="暂无已确认的外部转载记录" />
        <el-table v-else :data="detail.reposts">
          <el-table-column prop="site_name" label="网站" />
          <el-table-column prop="result_title" label="标题" min-width="260" />
          <el-table-column label="URL" min-width="320">
            <template #default="scope">
              <a :href="scope.row.canonical_url || scope.row.normalized_url" target="_blank" rel="noopener noreferrer">{{ scope.row.canonical_url || scope.row.normalized_url }}</a>
            </template>
          </el-table-column>
          <el-table-column prop="availability_status" label="链接状态" />
          <el-table-column prop="first_found_at" label="首次发现" min-width="160" />
        </el-table>
        <h3>搜索候选与人工复核</h3>
        <p class="note">仅保留标题相似度≥80%的候选；发布时间缺失、相同或更早均不影响候选保留。只有确认“外部转载”的链接会进入转载统计与 Excel；归入自有渠道的链接仅在“阅读量检测”中显示。</p>
        <el-empty v-if="!detail.candidates.length" description="尚无已保存候选" />
        <el-table v-else :data="detail.candidates">
          <el-table-column prop="searched_at" label="搜索时间" min-width="160" />
          <el-table-column prop="search_phases" label="阶段" min-width="110">
            <template #default="scope">{{ scope.row.search_phases.join(' / ') }}</template>
          </el-table-column>
          <el-table-column prop="site_name" label="网站" min-width="130" />
          <el-table-column prop="title" label="候选标题" min-width="240" />
          <el-table-column label="URL" min-width="300">
            <template #default="scope">
              <a :href="scope.row.canonical_url" target="_blank" rel="noopener noreferrer">{{ scope.row.canonical_url }}</a>
            </template>
          </el-table-column>
          <el-table-column prop="similarity_score" label="相似度" />
          <el-table-column label="复核状态" min-width="130">
            <template #default="scope">{{ relationLabel(scope.row.content_relation) }}</template>
          </el-table-column>
          <el-table-column prop="classification_reason" label="判定说明" min-width="180" />
          <el-table-column v-if="canOperate" label="人工复核" width="290">
            <template #default="scope">
              <el-button link type="primary" :disabled="Boolean(scope.row.manual_review_action)" @click="reviewCandidate(scope.row, 'CONFIRM_REPOST')">确认转载</el-button>
              <el-button link type="success" :disabled="Boolean(scope.row.manual_review_action)" @click="reviewCandidate(scope.row, 'CONFIRM_OWNED')">归入阅读量</el-button>
              <el-button v-if="scope.row.manual_review_action" link type="warning" @click="modifyCandidateReview(scope.row)">修改</el-button>
              <el-button v-if="!scope.row.manual_review_action" link type="danger" @click="reviewCandidate(scope.row, 'EXCLUDE')">排除</el-button>
            </template>
          </el-table-column>
        </el-table>
        <h3>搜索运行记录</h3>
        <el-table :data="detail.runs">
          <el-table-column prop="created_at" label="时间" min-width="160" />
          <el-table-column prop="provider" label="搜索服务" />
          <el-table-column prop="exact_candidate_count" label="EXACT" />
          <el-table-column prop="broad_candidate_count" label="BROAD" />
          <el-table-column prop="merged_candidate_count" label="合并候选" />
          <el-table-column prop="matched_count" label="已确认/匹配" />
          <el-table-column prop="repost_count" label="外部转载" />
          <el-table-column prop="review_required_count" label="待复核" />
          <el-table-column prop="error_message" label="异常" min-width="180" />
        </el-table>
      </template>
    </el-drawer>
    <el-dialog v-model="reviewDialogOpen" :title="reviewDialogTitle()" width="460px" :close-on-click-modal="false">
      <p class="review-note">{{ reviewingCandidate?.title }}</p>
      <el-form label-position="top">
        <el-form-item v-if="reviewMode === 'MODIFY'" label="修改为" required>
          <el-radio-group v-model="reviewAction">
            <el-radio value="CONFIRM_REPOST">外部转载</el-radio>
            <el-radio value="CONFIRM_OWNED">自有渠道（阅读量）</el-radio>
          </el-radio-group>
        </el-form-item>
        <el-form-item v-if="reviewAction === 'CONFIRM_OWNED'" label="自有渠道" required>
          <el-select v-model="reviewOwnedChannelId" placeholder="请选择已启用自有渠道" style="width: 100%">
            <el-option v-for="channel in ownedChannels" :key="channel.id" :label="channel.name" :value="channel.id" />
          </el-select>
        </el-form-item>
        <el-form-item :label="reviewMode === 'MODIFY' ? '修改原因' : '复核原因'" required>
          <el-input v-model="reviewReason" type="textarea" :rows="3" maxlength="500" show-word-limit :placeholder="reviewMode === 'MODIFY' ? '例如：复核后调整为自有渠道' : '例如：人工确认标题、来源与原创文章一致'" />
        </el-form-item>
      </el-form>
      <template #footer>
        <el-button :disabled="reviewSubmitting" @click="reviewDialogOpen = false">取消</el-button>
        <el-button type="primary" :loading="reviewSubmitting" @click="submitCandidateReview">{{ reviewMode === 'MODIFY' ? '保存修改' : '确认' }}</el-button>
      </template>
    </el-dialog>
  </section>
</template>
