...
type Article = { id: number; title: string; published_date: string; status: string; author_department: string; original_url: string; source_platform: string; notes: string };
...
const articleFilters = ref({ title: "", published_date_from: "", published_date_to: "", status: "" });
const articleForm = ref({ title: "", published_date: "", original_url: "", source_platform: "", author_department: "", notes: "" });
...
function resetArticle(): void { editingArticleId.value = null; articleForm.value = { title: "", published_date: "", original_url: "", source_platform: "", author_department: "", notes: "" }; }
...
async function loadArticles(): Promise<void> {
  ...
  articles.value = await api<Article[]>(`/articles${query.size ? `?${query.toString()}` : ""}`);
}
...
function editArticle(article: Article): void { editingArticleId.value = article.id; articleForm.value = { title: article.title, published_date: article.published_date, original_url: article.original_url, source_platform: article.source_platform, author_department: article.author_department, notes: article.notes }; }
...
<template>
  <main class="shell">
    ...
    <section v-else class="layout">
      ...
      <template v-if="page === 'articles'"><h1>原创文章</h1><el-form inline><el-form-item><el-input v-model="articleFilters.title" placeholder="标题查询" /></el-form-item><el-form-item><el-date-picker v-model="articleFilters.published_date_from" value-format="YYYY-MM-DD" type="date" placeholder="开始日期" /></el-form-item><el-form-item><el-date-picker v-model="articleFilters.published_date_to" value-format="YYYY-MM-DD" type="date" placeholder="结束日期" /></el-form-item><el-form-item><el-select v-model="articleFilters.status" clearable placeholder="状态"><el-option label="监测中" value="ACTIVE" /><el-option label="已归档" value="ARCHIVED" /></el-select></el-form-item><el-button @click="loadArticles">查询</el-button></el-form><el-form v-if="canOperate" class="editor" label-position="top" @submit.prevent="saveArticle"><el-form-item label="原创文章标题"><el-input v-model="articleForm.title" /></el-form-item><el-form-item label="原创发布日期"><el-date-picker v-model="articleForm.published_date" value-format="YYYY-MM-DD" type="date" /></el-form-item><el-form-item label="原创文章链接"><el-input v-model="articleForm.original_url" /></el-form-item><el-form-item label="原创发布平台"><el-input v-model="articleForm.source_platform" /></el-form-item><el-form-item label="作者/部门"><el-input v-model="articleForm.author_department" /></el-form-item><el-form-item label="备注"><el-input v-model="articleForm.notes" /></el-form-item><el-button type="primary" :loading="saving" @click="saveArticle">{{ editingArticleId ? '更新文章' : '新增文章' }}</el-button><el-button v-if="editingArticleId" @click="resetArticle">取消编辑</el-button></el-form><label v-if="canOperate" class="upload">Excel 导入预检<input type="file" accept=".xlsx" @change="uploadImport" /></label><el-alert v-if="importJob" :title="`预检：有效 ${importJob.valid_rows}，重复 ${importJob.duplicate_rows}，失败 ${importJob.failed_rows}`" type="info" show-icon><template #default><el-button size="small" :loading="saving" @click="confirmImport">导入有效行</el-button></template></el-alert><div v-if="canOperate && selectedArticles.length" class="toolbar"><el-button type="warning" @click="bulkArticleStatus('ARCHIVED')">归档已选 {{ selectedArticles.length }} 篇</el-button><el-button @click="bulkArticleStatus('ACTIVE')">恢复已选文章</el-button></div><el-empty v-if="!articles.length" description="暂无文章；请通过录入或 Excel 导入添加。" /><el-table v-else :data="articles" @selection-change="selectedArticles = $event"><el-table-column v-if="canOperate" type="selection" width="52" /><el-table-column prop="title" label="标题" min-width="220" /><el-table-column prop="published_date" label="发布日期" width="120" /><el-table-column prop="author_department" label="作者/部门" width="140" /><el-table-column prop="status" label="状态" width="100" /><el-table-column v-if="canOperate" label="操作" width="150"><template #default="scope"><el-button link @click="editArticle(scope.row)">编辑</el-button><el-button link type="danger" @click="archiveArticle(scope.row)">归档</el-button></template></el-table-column></el-table></template>
