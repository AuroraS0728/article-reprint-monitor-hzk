...
<template>
  <main class="shell">
    ...
    <section v-else class="layout">
      ...
      <template v-else-if="page === 'users'"><h1>用户与角色</h1><p class="note">创建用户需要管理员权限；初始密码至少 10 位，且不要使用过于常见的密码。</p><el-form class="editor" label-position="top" @submit.prevent="saveUser"><el-form-item label="用户名"><el-input v-model="userForm.username" :disabled="Boolean(editingUserId)" /></el-form-item><el-form-item label="邮箱"><el-input v-model="userForm.email" /></el-form-item><el-form-item :label="editingUserId ? '新密码（留空则不修改）' : '初始密码'"><el-input v-model="userForm.password" type="password" show-password placeholder="至少 10 位" /></el-form-item><el-form-item label="角色"><el-select v-model="userForm.role"><el-option label="管理员" value="ADMIN" /><el-option label="运营人员" value="OPERATOR" /><el-option label="查看人员" value="VIEWER" /></el-select></el-form-item><el-form-item label="启用"><el-switch v-model="userForm.is_active" /></el-form-item><el-button type="primary" :loading="saving" @click="saveUser">{{ editingUserId ? '更新用户' : '创建用户' }}</el-button><el-button v-if="editingUserId" @click="resetUser">取消编辑</el-button></el-form><el-empty v-if="!users.length" description="暂无用户。" /><el-table v-else :data="users"><el-table-column prop="username" label="用户名" /><el-table-column prop="email" label="邮箱" /><el-table-column prop="role" label="角色" /><el-table-column prop="is_active" label="启用" /><el-table-column label="操作"><template #default="scope"><el-button link @click="editUser(scope.row)">编辑</el-button></template></el-table-column></el-table></template>
