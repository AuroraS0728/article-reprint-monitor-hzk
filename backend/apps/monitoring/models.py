...
class DetectionResult(models.Model):
    batch = models.ForeignKey(
        DetectionBatch, related_name="results", on_delete=models.CASCADE
    )
    ...
    status = models.CharField(
        max_length=16,
        choices=PlatformDetectionStatus.choices,
        default=PlatformDetectionStatus.UNKNOWN,
    )
...
class StatusSnapshot(models.Model):
    ...
    source_batch = models.ForeignKey(
        DetectionBatch, null=True, blank=True, on_delete=models.PROTECT
    )
...
class TaskFailureLog(models.Model):
    ...
    batch = models.ForeignKey(
        DetectionBatch, null=True, blank=True, on_delete=models.SET_NULL
    )
