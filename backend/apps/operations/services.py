    ...
    try:
        for report in GeneratedReport.objects.filter(
            ...
            if _safe_delete_field_file(report.report_file):
                ...
                details["report_files_deleted"] = (
                    details.get("report_files_deleted", 0) + 1
                )

        for backup in DatabaseBackup.objects.filter(
            created_at__lt=now - timedelta(days=BACKUP_RETENTION_DAYS)
        ):
            _safe_delete_field_file(backup.backup_file)
            backup.delete()
            details["backup_records_deleted"] = (
                details.get("backup_records_deleted", 0) + 1
            )
        ...
        expiring_reports = GeneratedReport.objects.filter(
            generated_at__lt=now - timedelta(days=STATISTICS_RETENTION_DAYS),
            email_deliveries__isnull=True,
        )
        report_deleted, _ = expiring_reports.delete()
        details["report_records_deleted"] = report_deleted
        retained_snapshot_ids = GeneratedReport.objects.values_list(
            "snapshot_id", flat=True
        )
        snapshot_deleted, _ = (
            StatusSnapshot.objects.filter(
                created_at__lt=now - timedelta(days=STATISTICS_RETENTION_DAYS)
            )
            .exclude(id__in=retained_snapshot_ids)
            .delete()
        )
        details["statistics_snapshots_deleted"] = snapshot_deleted
        retained_batch_ids = StatusSnapshot.objects.exclude(
        ...
        protected_article_ids = set(
            DetectionResult.objects.values_list("article_id", flat=True)
        )
        ...
        legacy_repost_deleted, _ = RepostRecord.objects.filter(
            article_id__in=safe_articles
        ).delete()
