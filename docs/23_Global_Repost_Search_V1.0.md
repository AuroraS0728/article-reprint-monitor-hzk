...
原创来源所有子域名及 canonical 原文 URL 必须排除。搜索响应只有在标题相似度不低于 80%，且搜索服务提供的发布时间严格晚于原创发布时间时才会保存为候选；缺少发布时间、同一时间或更早的结果均不保存。最终自动确认仍使用 `SEARCH_SIMILARITY_THRESHOLD`（默认 90）的规则，80% 至 89.99% 的已保存候选仅供人工复核。
