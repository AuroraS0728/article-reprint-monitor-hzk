...
原创来源所有子域名及 canonical 原文 URL 必须排除。搜索响应只要标题相似度不低于 80% 就保存为候选；发布时间缺失、同一时间或更早均不影响候选保留，发布时间仅作为展示信息。最终自动确认仍使用 `SEARCH_SIMILARITY_THRESHOLD`（默认 90）的规则，80% 至 89.99% 的已保存候选仅供人工复核。
